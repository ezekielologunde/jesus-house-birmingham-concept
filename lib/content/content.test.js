...
describe("ministries", () => {
  ...
  it("uses the real group names from the church's official public groups directory", () => {
    expect(ministries.find((m) => m.id === "womens").name).toBe("Women of Purpose");
    expect(ministries.find((m) => m.id === "multimedia").name).toBe("Multi-Media");
    expect(ministries.find((m) => m.id === "ushering").name).toBe("Ushering");
    expect(ministries.find((m) => m.id === "prayer").name).toBe("Prayer");
    expect(ministries.find((m) => m.id === "food-bank").name).toBe("Food Bank");
    expect(ministries.find((m) => m.id === "welcome").name).toBe("Welcome & Follow-up");
  });
});
...
describe("giving", () => {
  ...
  it("links Give Online to the church's real domain, not a form on this site", () => {
    ...
    expect(online.href).toBe("https://jesushousebhm.onechurchsoftware.com/public/give");
  });
